...
(async () => {
  ...
  const h = await frame.$('h1, h2');
  const box = await h.boundingBox();
  const fb = await (await page.$('#clara-ve-frame')).boundingBox();
  await page.mouse.click(fb.x + box.x + Math.min(40, box.width / 2), fb.y + box.y + box.height / 2);
  ...
  out.click = [Math.round(fb.x + box.x + Math.min(40, box.width / 2)), Math.round(fb.y + box.y + box.height / 2)];
